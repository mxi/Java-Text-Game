		...
		LevelUp: for(;;)
		{
			if(character.xp >= character.level * character.level * 100)
			{
				...
				if(character.level % 4 == 0)
				{
					for(int x = 0; x < 2; x++)
					{
						MainGame.csi.print(0, 20, "Choose two stats to raise                 ");
						MainGame.csi.print(0, 21, "S: strength, D: dexterity                 ");
						MainGame.csi.print(0, 22, "C: constitiution, I: intelligence         ");
						MainGame.csi.print(0, 23, "W: wisdom, A: charisma                    ");
			...
			if(MainGame.csi.peekChar(CharMove.a + z, CharMove.b + Math.min(range - z, range + z)) == 'M')
			{
				for(int x = 0; x < curE; x++)
				{
					...
					if(m.x == CharMove.a + z && m.y == CharMove.b + Math.min(range - z, range + z))
					{
						...
						MainGame.csi.print(0, 20, "Monster took " + damage + " damage");
						MainGame.csi.print(0, 21, "Monster has " + monsters[x].hp + " health left");
						if(m.hp <= 0)
						{
							MainGame.csi.print(0, 22, "You killed it!");
							MainGame.csi.refresh();
