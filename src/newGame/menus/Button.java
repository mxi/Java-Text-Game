...
public class Button extends MenuComponent {
    ...
    protected void render(int color) {
        for(int ix = x; ix <= x + title.length() + 1; ix++) {
            MainGame.csi.print(parent.getSceneX() + ix, parent.getSceneY() + y, '*', color);
            MainGame.csi.print(parent.getSceneX() + ix, parent.getSceneY() + y + 2, '*', color);
        }
        MainGame.csi.print(parent.getSceneX() + x, parent.getSceneY() + y + 1, '*', color);
        MainGame.csi.print(parent.getSceneX() + x + title.length() + 1, parent.getSceneY() + y + 1, '*', color);
    ...
    protected void update(int key) {
        switch (key) {
            case 10: // enter
                if(onAction != null)
                    onAction.run();
                render(ConsoleSystemInterface.GRAY);
                try { Thread.sleep(290); }
                catch (InterruptedException e) { /* Do nothing */ }
                render(ConsoleSystemInterface.YELLOW);
                break;
            default:
                break;
        }
