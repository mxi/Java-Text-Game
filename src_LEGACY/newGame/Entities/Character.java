...
public class Character extends Entity {
    ...
    public int getGoldAmount() {
        int goldAmt = 0;
        if(inHand.sampleItem() != null && inHand.sampleItem() instanceof Gold)
            goldAmt += inHand.getSize();

        for(InventoryStack<Item> is : inventory)
            if(is != null && is.sampleItem() != null && is.sampleItem() instanceof Gold)
                goldAmt += is.getSize();
